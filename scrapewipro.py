...
a_values = []
date_value = []
base_url = "https://www.moneycontrol.com/stocks/company_info/stock_news.php?sc_id=W&scat=&pageno={}&next=0&durationType=Y&Year={}&duration=1&news_type="
for year in range(2011, 2024): # Change the range as per your requirement here the number of years are given
    ...
    for page_number in range(1,50):# here it goes on to next pages ie checks for values from 1 to 50
         ...
         if page_number==6 and year==2011:#here after 6th page in 2011 a blank page was left in the webpage so our code will break to avoid it
             continue
         if not job_tags and page_number!=3 and page_number!=1 and page_number!=2 and page_number!=4:
             #here after 3 a blank page was left in the webpage so our code will break to avoid it,and  page1 in 2019
...
with open('wiproenews.csv', 'w', newline='', encoding='utf-8') as file:
