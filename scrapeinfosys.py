...
base_url = "https://www.moneycontrol.com/stocks/company_info/stock_news.php?sc_id=IT&scat=&pageno={}&next=0&durationType=Y&Year={}&duration=1&news_type="
...
for year in range(2011, 2024): # Change the range as per your requirement here the number of years are given
    ...
    for page_number in range(1,50):# here it goes on to next pages ie checks for values from 1 to 50
         ...
         if not job_tags and page_number!=3 and page_number!=1 and page_number!=2 :  # If the page has no matching tags, break the loop and move to the next year,without this the code took so long :(
...
with open('infosysnews.csv', 'w', newline='', encoding='utf-8') as file:
